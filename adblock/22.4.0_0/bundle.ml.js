...
function processElement(e) {
  let { adjacencyMatrix: t, features: n } = addEdgesFeatures(e, GRAPH_CUT_OFF);
...
for (let e in REQUEST_TYPES) requestTypesDict[REQUEST_TYPES[e]] = reqtypeVEC[e];
const URL_CHARS =
...
let featureGenerator = function () {
  ...
  let a = function (t) {
    ...
    for (let t in CSS_COLORS) {
      let n = r.hasOwnProperty(CSS_COLORS[t]) ? o(r[CSS_COLORS[t]]) : [0, 0, 0];
      e(l, n);
    }
    ...
    for (let t in CSS_ORIGINS) {
      let n = r.hasOwnProperty(CSS_ORIGINS[t]) ? s(r[CSS_ORIGINS[t]]) : [0, 0];
      e(l, n);
    }
    return l;
  };
